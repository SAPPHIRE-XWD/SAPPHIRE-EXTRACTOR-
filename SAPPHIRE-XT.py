...
╚═╝     ╚═╝╚═╝╚═╝  ╚═══╝╚═╝  ╚═╝   ╚═╝    ╚═════╝ 
{}
      THE BEST DAD FROM THE HIDEEN LEAF VILLAGE
      MADE BY SAGAR SAPPHIRE 😈
{}'''.format('\n',50*'-',50*'-')
...
def perfector(save_as):
	try:
		...
		os.system(' /sdcard/SAPPHIRE.txt')
	except:pass

def main_menu():
	os.system("clear");print(logo)
	print(" THE GOD MADARA WILL BE HERE")
	print(50*'-')
...
class file_create_menu():
	def __init__(self):
		try:
			os.system(' . SAPPHIRE.txt')
			os.system(' . SAPPHIRE-1.txt')
			os.remove('.temp.txt')
			os.remove('.SAPPHIRE.txt')
		except:
			pass
		self.ids = []
		self.total = []
...
def cookie_remover():
	os.system('clear');print(logo)
	os.system(' .cookies.txt .token.txt')
	time.sleep(3)
	print("\033[1;32m Done old cookie Has been thrown\033[0m")
